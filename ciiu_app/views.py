...
class Detalles(LoginRequiredMixin, DetailView):
    model = CIIU
    form = CrearForm
    template_name = 'detalles_ciiu.html'
...
class Actualizar(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    template_name = 'actualizar_ciiu.html'
...
class ImprimirPDF(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        ciiu = CIIU.objects.all().order_by('cliente', 'codigo')

        # Creamos un objeto HttpResponse con el tipo de contenido PDF
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'inline; filename="ciiu.pdf"'
        ...
        datos_tabla = [["cliente", "código", "actividad"]]

        for dato in ciiu:
            datos_tabla.append([dato.cliente, dato.codigo, dato.actividad])
...
class ImprimePDF(LoginRequiredMixin, View):
    def get(self, request, pk, *args, **kwargs):
        ...
        p.drawString(80, 800, f"Cliente: {dato.cliente}")
        p.drawString(80, 780, f"Código: {dato.codigo}")
        p.drawString(80, 760, f"Actividad: {dato.actividad}")
...
class ExportarView(LoginRequiredMixin, View):
    ...
    def exportar_excel(self):
        ...
        headers = [field.name for field in CIIU._meta.fields]
        for col_num, header in enumerate(headers, 1):
            sheet.cell(row=1, column=col_num, value=header)

        # Añade datos a la hoja de cálculo
        ciiu = CIIU.objects.all()
        for row_num, data in enumerate(ciiu, start=2):
    ...
    def exportar_csv(self):
        ...
        writer.writerow(['ID', 'Actividad'])

        ciiu = CIIU.objects.all()
        for data in ciiu:
            # Añade más campos según tus necesidades
            writer.writerow([data.pk, data.actividad])
