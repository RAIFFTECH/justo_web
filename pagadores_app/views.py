...
class Crear(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def get_initial(self):
        ...
        # Tomar cliente desde URL o sesión
        cliente_id = self.kwargs.get('cliente_id') or self.request.session.get('cliente_id')
        if not cliente_id:
            cliente_id = 1  # Por defecto si no hay en URL/sesión
        # cliente_id = 1  # Aquí pones el cliente por defecto o lo tomas de la URL/sesión
        ultimo = PAGADORES.objects.filter(cliente_id=cliente_id).order_by('-codigo').first()

        if ultimo and ultimo.codigo.isdigit():
            siguiente_codigo = str(int(ultimo.codigo) + 1).zfill(5)
        else:
            siguiente_codigo = '00001'

        initial['codigo'] = siguiente_codigo
